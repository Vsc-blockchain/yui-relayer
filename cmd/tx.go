...
func relayMsgsCmd(ctx *config.Context) *cobra.Command {
	...
	cmd := &cobra.Command{
		...
		RunE: func(cmd *cobra.Command, args []string) error {
			...
			if m, err := st.UpdateClients(cmd.Context(), c[src], c[dst], doExecuteRelay, doExecuteRelay, doExecuteAckSrc, doExecuteAckDst, sh, viper.GetBool(flagDoRefresh)); err != nil {
				return err
			} else {
				msgs.Merge(m)
			}

			if m, err := st.RelayPackets(cmd.Context(), c[src], c[dst], sp, sh, doExecuteRelay, doExecuteRelay); err != nil {
				return err
			} else {
				msgs.Merge(m)
			}
