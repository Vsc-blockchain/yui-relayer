...
func (st *NaiveStrategy) RelayPackets(ctx context.Context, src, dst *ProvableChain, rp *RelayPackets, sh SyncHeaders, _, _ bool) (*RelayMsgs, error) {
	logger := GetChannelPairLogger(src, dst)
...
func collectPackets(srcCtx, dstCtx QueryContext, src, dst *ProvableChain, srcPackets, dstPackets PacketInfoList, srcSigner, dstSigner sdk.AccAddress) ([]sdk.Msg, []sdk.Msg, error) {
	...
	dstHeader, err := dst.Prover.GetLatestFinalizedHeader(dstCtx.Context())
	if err != nil {
	...
	dstTimestamp, err := dst.Timestamp(dstCtx.Context(), dstHeight)
	if err != nil {
		logger.Error("failed to get latest timestamp", err)
		return nil, nil, err
	}
	srcHeader, err := src.Prover.GetLatestFinalizedHeader(srcCtx.Context())
	if err != nil {
		logger.Error("failed to get latest header", err)
	}
	...
	srcTimestamp, err := src.Timestamp(srcCtx.Context(), dstHeight)
	if err != nil {
		logger.Error("failed to get latest timestamp", err)
		return nil, nil, err
	}
